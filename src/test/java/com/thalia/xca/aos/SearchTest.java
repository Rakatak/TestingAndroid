	...
	public void searchHistoryTest() throws Exception {
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				// Check the SearchHistory, re-enter the searchInput via
				// Dropdown menu and checking textvalue
				element.click();

				eName = "eu.thalia.app:id/search_close_btn";
				element = wd.findElementById(eName);
				element.click();
				Thread.sleep(4000);
				...
				eName = "eu.thalia.app:id/search_src_text";
				String result = wd.findElementById(eName).getAttribute("text");

				assertTrue(
						"The input: ("
								+ searchInput
								+ ") wasn't displayed correctly in the history. Result: "
								+ result, result.contains(searchInput));
			}
		};
	...
	public void searchCategoryTest() throws Exception {
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				assertTrue(
						"The search category: (eBooks) wasn't displayed/selected correctly ",
	...
	public void searchCategoryResultTest() throws Exception {
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				assertTrue("The search category: (" + check
						+ ") wasn't displayed/selected correctly : " + result,
						result.contains(check));
			}
		};
		action.performAction();
	...
	public void searchOptionsTest() throws Exception {
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				List<MobileElement> list = wd
						.findElementsById("eu.thalia.app:id/filter_option_label");
				int checker = 0;
				...
				assertTrue("The search option: (" + check
						+ ") wasn't displayed/selected correctly ", checker > 0);
			}
		};
		action.performAction();
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				for (int i = 0; i < checkList.size(); i++) {
					if (i == 4 && aCap.screenHeight == AppiumSetup.motoHeight) {
				...
				assertTrue("The search categories are incomplete" + checker
						+ " of " + checkList.size(),
						checker == checkList.size());
			}
		};
		action.performAction();
	...
	public void checkSearchOptions() throws Exception {
		...
		AbstractExcAction action = new AbstractExcAction(wd) {
			...
			public void actionPerformedWithThrows()
				...
				// Check the total number
				assertTrue("The search options are incomplete: " + checker
						+ " of " + checkList.size(),
						checker == checkList.size());
			}
		};
